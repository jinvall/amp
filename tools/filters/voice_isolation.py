#!/usr/bin/env python3
"""Voice isolation filter for AMP Silver tools.

Isolates the human voice band while attenuating everything else. Useful for
speech enhancement, podcast cleanup, or extracting vocals from a mix.
"""
...
class VoiceIsolationFilter(BaseFilter):
    """Isolate voice band via spectral gating.

    Parameters:
        voice_low_hz: lower edge of voice band (default 80)
        voice_high_hz: upper edge of voice band (default 4000)
        preserve_db: how much of the voice band to keep (default 0)
        attenuate_db: how much to cut non-voice frequencies (default 18)
        window_size: FFT window size (default 2048)
    """

    def __init__(self, sample_rate=44100, channels=1,
                 voice_low_hz=80.0, voice_high_hz=4000.0,
                 preserve_db=0.0, attenuate_db=18.0, window_size=2048):
        super().__init__(sample_rate, channels)
        self.voice_low_hz = float(voice_low_hz)
        self.voice_high_hz = float(voice_high_hz)
        self.preserve_db = float(preserve_db)
        self.attenuate_db = float(attenuate_db)
    ...
    def process(self, pcm_bytes):
        ...
        with self._lock:
            ...
            while len(self._input_buffer) >= self.window_size:
                ...
                gain[voice_band] = 10 ** (-self.preserve_db / 20.0)
                gain[~voice_band] = 10 ** (-self.attenuate_db / 20.0)
